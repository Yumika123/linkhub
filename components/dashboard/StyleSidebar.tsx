...
export function StyleSidebar({
  ...
  const initialBackgroundRef = React.useRef(background);

  React.useEffect(() => {
    if (isOpen) {
      console.log("[StyleSidebar] Capturing initialBackground:", background);
  ...
  const handleCancel = () => {
    reset(parseBackground(initialBackgroundRef.current));
    onBackgroundChange(initialBackgroundRef.current);
    onClose();
  };
